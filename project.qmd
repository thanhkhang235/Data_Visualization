...
correlation_matrix <- cor(merged_data_2[, c("Population", "Accidents")])
...
```{r}
ggplot(merged_data_2, aes(x = Population, y = Accidents, size = Accidents, color = Accidents)) +
...
```{r}
df <- left_join(data, merged_data_2, by = c("state" = "State") )
